...
library(moments) #medidas estadiscas
...
En este gráfico se muestra un mapa de calor por departamentos en función de ciertos intervalos que representan el número de homicidios a lo largo de la historia del país.

En este mapa se puede observar como Antioquia es el departamento con mayor número de homicidios con más de 125000 homicidios registrados en su historia, también se observa que el segundo departamento con mayor número de homicidios fué Valle del Cauca con más de 50000 homicidios. Adicional a esto se observa que la zona norte del país tuvo un mayor índice de homicidios que el resto del país
...
En este gráfico se hizo una representación binaria, comparando las cifras que se le atribuyen a grupos paramilitares con las que se le atribuyen a las guerrillas. Los departamentos en verde son aquellos en los que las guerrillas tienen mayor cifra de homicidios que los paramilitares, y los departamentos en amarillo son aquellos en los que los paramilitares alcanzaron mayores cifras que las guerrillas. 

Si bien la zona que corresponde a grupos paramilitares en el mapa, al compararla con el mapa de calor, tiene mayores cifras de homicidios, las guerrillas tienen mayor impacto en mayor número de departamentos
...
En este grafico se muestra un mapa de calor por departamentos en función de ciertos intervalos que representan el número de Secuestros a lo largo de la historia del país.

 Se puede observar como una vez más Antioquia es el departamento más afectado. En este caso el mapa de calor presenta mayor uniformidad, sin embargo el norte del país sigue teniendo un gran número de casos con respecto al resto de país.
...
En este grafico se hizo una representación binaria, comparando las cifras que se le atribuyen a grupos paramilitares con las que se le atribuyen a las guerrillas. Los departamentos en verde son aquellos en los que las guerrillas tienen mayor cifra de secuestros que los paramilitares , y los departamentos en amarillo son aquellos en los que los paramilitares alcanzaron mayores cifras que las guerrillas. 

Se puede observar como las guerrillas tienen mayor impacto en el mapa en tema de secuestros que los paramilitares, siendo que , los paramilitares solo tuvieron más impacto que las guerrillas en dos departamentos.
...
En este gráfico se muestra un mapa de calor por departamentos en función de ciertos intervalos que representan el número de reclutamientos a lo largo de la historia del país.

Se puede observar como Antioquia es un departamento en el que suelen haber muchos reclutamientos, sin embargo, el departamento con mayor número de reclutamientos fué el Meta. Además se puede observar como la zona centro-sur del país fue la que se vió mas afectada para este caso.


### Secuestros que llevan a homicidios

Hay una relación entre varios registros de diferentes dataframes de un mismo grupo por id.

Tomando estos datos y los datos totales se puede decir que cerca del 12% de los secuestros efectuados por las guerrillas terminan en homicidios mientras que cerca del 28% de secuestros realizados por los paramilitares terminan en homicidios

A continuacion se muestra la respectiva tabla.
...
El dataframe generado muestra el numero de secuestros que terminaron en homicidio por parte de Las guerrillas y los paramilitares a lo largo de la historia.

```{r echo=FALSE}
knitr::kable(
  head(kidnapping_homicide_per_group),
  caption = "Homicidios que llevan a secuestros",
  table.attr = 'class="table table-striped table-bordered"',
  align = "c",
  format = "html"
...
## Datos por año
En general la violencia en el margen del conflicto armado aumentó hasta llegar a su punto máximo en el año 2003 para luego decrecer. De esto se pueden decir varias cosas:
 
1. Se desconoce si en los primeros años registrados en los datasets se omitió información, ya sea por las limitaciones de la época para mantener registros de los datos o contexto social.

2. Al decrecimiento de las tendencias se le pueden atribuir distintos factores, pero un factor considerable puede ser la desmovilización de los paramilitares entre 2003 y 2006.

El homicidio fue el hecho mas frecuente a lo largo de los años, siendo tan frecuente que se acerca mas al total de los hechos que al segundo hecho mas frecuente.
 
 
## Datos por grupo
 
* En lo que respecta a homicidios y desapariciones los paramilitares tuvieron las mayores cifras. Este numero de cifras respalda la creencia popular sobre el hecho de que los paramilitares eran mas propensos a cometer homicidios.

* En lo referente a secuestros las guerrillas tuvieron mayores cifras que los paramilitares y cualquier otro grupo. Esto indica que la creencia popular de que las guerrillas son propuestas a realizar secuestros es acorde a los datos tomados. Ademas se puede complementar esta información con los hechos históricamente conocidos de que las guerrillas encontraban financiamiento en los secuestros

* Como se mencionó en el inciso anterior, la desmovilización de los paramilitares pudo ser un factor muy influyente en la disminución de la violencia, Sin embargo, cuando ocurrió este hecho y en ese mismo aumentaron los hechos atribuidos a otros grupos desconocidos. Estos grupos desconocidos pueden ser tantos disidencias como nuevos grupos que surgieron tras el vacío de participación de los paramilitares.

## Datos por departamento
 
En este caso ocurrió algo sorpresivo en lo que respecta a secuestros y homicidios se puede decir que las guerrillas tuvieron mas impacto en el país que los paramilitares, por lo tanto se puede hallar una explicación sobré porqué existe mayor rechazo hacia las guerrillas que hacia los paramilitares. Entonces cabe preguntarse ¿Porqué si las guerrillas tuvieron mayor impacto en un numero de departamentos los paramilitares tienen mayores cifras en lo que respecta a hechos violentos?, la respuesta radica en que, aunque las guerrillas tuvieron predominancia en la mayoría de departamentos y ,en tema de secuestros tuvieron predominancia en casi todo el país, los paramilitares tuvieron cifras demasiado elevadas en los departamentos en los que tuvieron predominancia como por ejemplo el departamento de Antoquia, el cual podría ser considerado "zona roja" , dado que fue el departamento con mayor indice de violencia entre todos los departamentos.

Sobre los secuestros que llevan a homicidios se pueden decir dos cosas:

1. Si bien las guerrillas eran mas propensas a cometer secuestros, es mas probable que ese secuestro no lleve a un homicidios que un secuestro realizado por paramilitares.

2. En cuestión de cifras los secuestros que llevan a homicidios no representan gran parte de los hechos, siendo que solo 9200 secuestros terminaron en homicidios.

## Informacion que no cubre este informe.

* No se consideró ni el sexo ni la etnia.
* No se considerarón las imputaciones de la JEP puesto que no es el objetivo del informe.
* Se desconocen las limitaciones del conjunto de datos
